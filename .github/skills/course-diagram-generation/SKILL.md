...
1. Use linear flow for staged pipelines.
2. Use circular flow for lifecycle loops only when a loop is explicitly stated.
3. Use hub-and-spoke for central platform models.
4. Use split-lane layout for comparisons.
5. Use feedback arrows only when feedback is explicitly stated in slide content.
6. If structure is ambiguous, keep the slide text-based and document why no diagram was added.
