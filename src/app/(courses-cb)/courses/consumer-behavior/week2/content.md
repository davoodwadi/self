---
topic: Perception and Sensory Marketing
lecturer: "Davood Wadi, PhD"
course: Consumer Behavior
week: week2
---

## Title Slide
- Week 02
- We do not see things as they are. We see things through our senses and personal filters.

## What Is Perception? [no-quiz]
- Perception is the process of selecting, organizing, and interpreting sensations.
- Raw sensory inputs reach our sensory organs every second.
- Sights, sounds, smells, tastes, and textures flood our environment.
- Perception creates meaning out of this chaos.
- Two people exposed to the exact same advertisement often walk away with completely different impressions.

## The Three Stages of Perception [quiz]
[figure: PerceptionFunnel — the three sequential stages: Exposure, Attention, and Interpretation]
- Perception occurs in three sequential stages.
- Stage 1 is Exposure. Sensory stimuli come within the range of a person's sensory receptors.
- Stage 2 is Attention. The mind allocates mental processing capacity to a specific stimulus.
- Stage 3 is Interpretation. The brain assigns meaning to the sensory stimulus based on past experiences and expectations.
- Most stimuli get filtered out during exposure and attention before reaching interpretation.

## Sensory Marketing: Beyond Sight and Sound
[figure: SensoryWheel — the five sensory modalities in marketing: vision, audition, olfaction, haptics, and gustation]
- Sensory marketing engages the consumer's senses to influence their feelings and choices.
- Sight creates visual identity, package recognition, and color associations.
- Sound shapes shopping tempo and brand memory through musical jingles.
- Smell connects directly to the limbic system, triggering emotional memories.
- Touch, also called haptics, builds psychological ownership when consumers hold an item.
- Taste drives loyalty in food and beverage markets through distinct flavor profiles.

## Sensory Thresholds: The Limits of Awareness [quiz]
- Senses have physical and psychological limits.
- The absolute threshold is the minimum amount of stimulation a person can detect on a sensory channel.
- A billboard with tiny text placed far from a highway falls below the absolute threshold of drivers.
- The differential threshold is the ability of a sensory system to detect changes or differences between two stimuli.
- The minimum difference between two stimuli needed for detection is called the Just Noticeable Difference, or JND.

## Weber's Law: When Differences Matter [quiz]
[figure: WebersLawScale — Weber's Law demonstrating proportional change needed for detection in package updates vs price increases]
- Weber's Law states that the stronger the initial stimulus, the greater the change must be for people to notice it.
- The needed change is not a fixed amount. It is a constant proportion of the initial intensity.
- A ten-cent price increase on a one-dollar candy bar is noticeable right away because it is a ten percent jump.
- A ten-cent price increase on a thousand-dollar laptop will go completely unnoticed.
- Marketers stay below the JND when shrinking package sizes or raising prices slightly.
- Marketers exceed the JND when redesigning a package or announcing a major discount.

## Attention: The Battle for Mental Energy
- Consumers face sensory overload from thousands of ads, notifications, and store displays every day.
- Selective exposure means consumers choose what media and messages they see or avoid.
- Perceptual vigilance means consumers notice stimuli that relate to their current needs. A hungry person notices restaurant signs.
- Perceptual defense means consumers screen out threatening or contradictory messages. Heavy smokers often ignore warning labels on cigarette packs.
- Adaptation occurs when consumers no with time stop paying attention to familiar stimuli.

## Interpretation and Gestalt Principles [quiz]
[figure: GestaltPrinciples — Closure, Similarity, and Figure-Ground visual examples in brand design]
- People do not interpret stimuli in isolation. They organize them into patterns.
- Gestalt psychology explains how people construct whole meanings from individual elements.
- The Closure principle means people fill in missing pieces to perceive a complete picture, such as an incomplete brand logo.
- The Similarity principle means consumers group objects with shared visual traits like color and shape together.
- The Figure-Ground principle means people separate a main focal object from its background.

## Semiotics: The Meaning of Marketing Messages
- Marketers design ads to send symbolic messages to buyers.
- Semiotics is the study of signs, symbols, and their assigned meanings.
- An object is the actual product being promoted.
- A sign is the sensory image or symbol representing the intended meaning.
- An interpretant is the meaning or feeling derived by the consumer from the sign.
- A luxury watch ad uses an image of an eagle to signify freedom and prestige.

## Discussion: Sensory Signatures
- Discussion: Name a brand that you recognize instantly without seeing its name or logo. Does it use a signature scent, a distinct jingle, a unique bottle shape, or a specific color? How does that sensory cue build brand recall?
