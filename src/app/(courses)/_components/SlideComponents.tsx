...
// ============================================================================
// Data-driven visualization components to replace Mermaid diagrams

/**
 * FlowChart - Systematic, data-driven flowchart component
 *
 * PURPOSE:
 * - Replace Mermaid diagrams with React-based, cinematic flowcharts
 * - Handle various flow patterns: linear, branching, converging, circular
 * - Provide consistent, interactive visualizations
 *
 * WHEN TO USE:
 * - Process flows, pipelines, lifecycles
 * - Decision trees or branching logic
 * - System architecture diagrams
 * - Any node-edge graph visualization
 *
 * FEATURES:
 * - Multiple layouts (horizontal, vertical, circular)
 * - Labeled edges for annotated connections
 * - Group/subgraph support for logical grouping
 * - Responsive design with mobile/desktop variants
 * - Interactive hover effects
 *
 * PROPS:
 * @param nodes - Array of {id, label, icon?, group?}
 * @param edges - Array of {from, to, label?}
 * @param layout - 'horizontal' | 'vertical' | 'circular' | 'elk' | 'elk-horizontal' | 'elk-vertical' (default: 'horizontal')
 * @param caption - Optional caption below diagram
 * @param className - Additional Tailwind classes
 *
 * EXAMPLE:
 * <FlowChart
 ...
 *   edges={[
 *     { from: 'input', to: 'quant' },
 *     { from: 'quant', to: 'output' }
 *   ]}
 *   layout="horizontal"
 *   caption="Model compression pipeline"
 * />
 *
 * COMPLEX EXAMPLE (with branching and labels):
 * <FlowChart
 *   nodes={[
 *     { id: 'P', label: 'Production' },
 *     { id: 'C', label: 'Consumption' },
 *     { id: 'W', label: 'Waste' },
 *     { id: 'R', label: 'Recycling' }
 *   ]}
 *   edges={[
 *     { from: 'P', to: 'C' },
 *     { from: 'C', to: 'W' },
 ...
 *   ]}
 *   layout="circular"
 *   caption="Circular economy with AI"
 * />
 */
export function FlowChart({
  ...
  const hasGroupedNodes = useMemo(
    () => nodes.some((node) => Boolean(node.group)),
    [nodes],
  );

  const useElkLayout =
    (layout === "elk" && !hasLoopback) ||
  ...
  const isVerticalDirection =
    layout === "vertical" || (useElkLayout && elkDirection === "DOWN");
  ...
  const CinematicNode = memo(function CinematicNode({
    data,
  }: NodeProps<Node<FlowNodeData>>) {
    return (
      <div className="relative flex flex-col items-center justify-center min-w-[154px] min-h-[100px] px-4 py-3 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(249,247,245,0.92))] border border-[var(--charcoal)]/12 rounded-[22px] shadow-[0_12px_28px_rgba(26,26,29,0.12)] transition-all duration-500 hover:border-[var(--crimson)]/55 hover:shadow-[0_18px_36px_rgba(139,0,0,0.16)] hover:-translate-y-0.5 group">
        <Handle
          type="target"
          id="target-top"
        ...
        {data.icon ? (
          <div className="mb-2 flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--crimson)]/20 bg-[radial-gradient(circle_at_top,rgba(139,0,0,0.14),rgba(255,255,255,0.96)_70%)] text-[var(--charcoal)] transition-all duration-500 group-hover:scale-110 group-hover:border-[var(--crimson)]/40 group-hover:text-[var(--crimson)] [&_svg]:h-5 [&_svg]:w-5">
            {data.icon}
          </div>
        ) : null}

        <div className="text-base font-serif font-semibold text-[var(--charcoal)] text-center leading-tight">
          {data.label}
        </div>

        {data.group ? (
          <div className="text-[10px] tracking-wider uppercase text-[var(--crimson)] mt-2">
            {data.group}
          </div>
        ) : null}

        {data.hasManyConnections ? (
          <div className="absolute -top-2 -right-2 w-6 h-6 bg-[var(--crimson)] text-[var(--surface)] text-xs rounded-full flex items-center justify-center font-bold">
            +
          </div>
        ) : null}
  ...
  const CinematicEdge = memo(function CinematicEdge({
    id,
    sourceX,
    sourceY,
    targetX,
    ...
    const isCircular = Boolean(edgeData.isCircular);
    const isLoopback = Boolean(edgeData.isLoopback);
    const hasLabel = Boolean(edgeData.label);
    const prefersOuterArc = isCircular && (isLoopback || hasLabel);
    ...
    if (prefersOuterArc) {
      ...
      const fallbackCenterX = (sourceX + targetX) / 2;
      const fallbackCenterY = (sourceY + targetY) / 2;
      const graphCenterX = edgeData.graphCenterX ?? fallbackCenterX;
      const graphCenterY = edgeData.graphCenterY ?? fallbackCenterY;

      const midX = (sourceX + targetX) / 2;
      const midY = (sourceY + targetY) / 2;
      let outwardX = midX - graphCenterX;
      let outwardY = midY - graphCenterY;
      const outwardMagnitude = Math.hypot(outwardX, outwardY);

      if (outwardMagnitude < 1) {
        outwardX = -dy;
        outwardY = dx;
      }

      const normalizedMagnitude = Math.hypot(outwardX, outwardY) || 1;
      outwardX /= normalizedMagnitude;
      outwardY /= normalizedMagnitude;

      const arcStrength = isLoopback
        ? Math.max(120, Math.min(220, distance * 1.1))
        : Math.max(76, Math.min(150, distance * 0.74));
      const minX = graphCenterX - 315;
      const maxX = graphCenterX + 315;
      const minY = graphCenterY - 235;
      const maxY = graphCenterY + 235;

      const control1X = clamp(sourceX + outwardX * arcStrength, minX, maxX);
      const control1Y = clamp(sourceY + outwardY * arcStrength, minY, maxY);
      const control2X = clamp(targetX + outwardX * arcStrength, minX, maxX);
      const control2Y = clamp(targetY + outwardY * arcStrength, minY, maxY);

      edgePath = `M ${sourceX},${sourceY} C ${control1X},${control1Y} ${control2X},${control2Y} ${targetX},${targetY}`;

      const t = isLoopback ? 0.5 : 0.46;
      const oneMinusT = 1 - t;
      labelX =
        oneMinusT ** 3 * sourceX +
      ...
      const outwardLabelOffset = isLoopback ? 18 : 12;
      labelX += outwardX * outwardLabelOffset;
      labelY += outwardY * outwardLabelOffset;

      labelX = clamp(labelX, minX + 34, maxX - 34);
      labelY = clamp(labelY, minY + 16, maxY - 16);
    } else {
      const [path, computedLabelX, computedLabelY] = getBezierPath({
        sourceX,
        sourceY,
        targetX,
        targetY,
        sourcePosition,
        targetPosition,
        curvature: isCircular ? 0.42 : 0.35,
      });
      edgePath = path;
    ...
    const primaryStroke = isLoopback ? "var(--gold)" : "var(--crimson)";
    const primaryWidth = isLoopback ? 3 : isCircular ? 2.3 : 2;
    const primaryDash = isCircular ? undefined : "6 4";
    const labelDx = isCircular ? 0 : 4;
    const labelDy = isLoopback ? -8 : isCircular ? -6 : -8;
    const markerId = isLoopback
      ? "url(#cinematic-arrow-loop)"
      : "url(#cinematic-arrow)";
    ...
    return (
      <>
        <BaseEdge
          ...
          path={edgePath}
          style={{
            stroke: "rgba(249, 247, 245, 0.95)",
            strokeWidth: primaryWidth + 2.4,
            opacity: isCircular ? 0.56 : 0.45,
          }}
        />
        <BaseEdge
          id={id}
        ...
        {edgeData.label ? (
          <EdgeLabelRenderer>
            <div
              className="text-xs font-semibold px-2.5 py-1 rounded-md border shadow-[0_4px_18px_rgba(26,26,29,0.14)]"
              style={{
                position: "absolute",
  ...
  const heuristicNodes = useMemo(() => {
    const degrees = new Map<string, { in: number; out: number }>();

    nodes.forEach((node) => degrees.set(node.id, { in: 0, out: 0 }));
    edges.forEach((edge) => {
      const sourceDegree = degrees.get(edge.from);
      const targetDegree = degrees.get(edge.to);
      if (sourceDegree) sourceDegree.out += 1;
      if (targetDegree) targetDegree.in += 1;
    });

    const sources = nodes.filter((n) => (degrees.get(n.id)?.in ?? 0) === 0);
    const sinks = nodes.filter((n) => (degrees.get(n.id)?.out ?? 0) === 0);
    ...
    if (useCircularLayout) {
      const centerX = 360;
      const centerY = 190;
      const radius = Math.max(190, Math.min(250, nodes.length * 46));
    ...
    } else if (Object.keys(groupedNodes).length > 1) {
      const groups = Object.entries(groupedNodes);
      groups.forEach(([groupName, groupNodes], groupIndex) => {
        const startX = 100 + groupIndex * 360;
        const startY = 80;
        groupNodes.forEach((node, rowIndex) => {
          positions.set(node.id, {
            x: startX,
            y: startY + rowIndex * 170,
          });
        });
        if (groupName === "default") return;
      });
    } else if (sources.length === 1 && sinks.length === 1 && nodes.length > 3) {
      // Diamond-style layout: source -> middle row -> sink
      const source = sources[0];
      const sink = sinks[0];
      const middle = nodes.filter(
        (n) => n.id !== source.id && n.id !== sink.id,
      );

      const centerX = 360;
      positions.set(source.id, { x: centerX, y: 25 });

      const middleSpacing = 220;
      const middleStartX = centerX - ((middle.length - 1) * middleSpacing) / 2;
      middle.forEach((node, index) => {
        positions.set(node.id, {
          x: middleStartX + index * middleSpacing,
          y: 200,
        });
      });

      positions.set(sink.id, { x: centerX, y: 375 });
    } else if (sources.length === 1) {
      // Diverging layout
      const source = sources[0];
      positions.set(source.id, { x: 120, y: 190 });
      ...
      // Converging layout
      const sink = sinks[0];
      positions.set(sink.id, { x: 500, y: 190 });

      const startNodes = nodes.filter((n) => n.id !== sink.id);
      startNodes.forEach((source, index) => {
      ...
      // Grid fallback for complex graphs
      nodes.forEach((node, index) => {
        const col = index % 3;
    ...
    return nodes.map((node) => {
      ...
      return {
        ...
        type: "cinematic",
        position,
        sourcePosition: isVerticalDirection
          ? Position.Bottom
          : useCircularLayout
            ? Position.Right
            : Position.Right,
        targetPosition: isVerticalDirection
          ? Position.Top
          : useCircularLayout
            ? Position.Left
            : Position.Left,
        data: {
          ...
          icon: node.icon,
          group: node.group,
          hasManyConnections: degree.in > 1 || degree.out > 1,
        },
        style: {
          width: nodeWidth,
          height: nodeHeight,
  ...
  }, [edges, isVerticalDirection, nodes, useCircularLayout]);
  ...
  }, [edges, elkDirection, nodeHeight, nodeWidth, nodes, useElkLayout]);

  const positionedNodes = useMemo(() => {
    return heuristicNodes.map((node) => {
      const elkPosition = elkPositions?.get(node.id);

      return {
        ...node,
        position: useElkLayout && elkPosition ? elkPosition : node.position,
        sourcePosition: isVerticalDirection
          ? Position.Bottom
          : useCircularLayout
            ? Position.Right
  ...
  const flowEdges = useMemo(() => {
    ...
    const resolveClosestHandles = (
      ...
      const getAnchor = (position: { x: number; y: number }, side: Side) => {
        const centerX = position.x + nodeWidth / 2;
        const centerY = position.y + nodeHeight / 2;

        if (side === "top") return { x: centerX, y: position.y };
        if (side === "right") return { x: position.x + nodeWidth, y: centerY };
        if (side === "bottom")
          return { x: centerX, y: position.y + nodeHeight };
        return { x: position.x, y: centerY };
      };

      let bestPair: { source: Side; target: Side; distance: number } = {
        source: "right",
        target: "left",
      ...
      return {
        sourceHandle: `source-${bestPair.source}`,
        targetHandle: `target-${bestPair.target}`,
      };
    };

    const resolveCircularHandles = (
      source: { x: number; y: number },
      target: { x: number; y: number },
    ) => {
      ...
      // Prefer tangential side pairs for circular readability.
      if (dx >= 0 && dy >= 0) {
    ...
    const isCircularGraph = useCircularLayout;

    return edges.map((edge, index) => {
      const sourceIndex = nodeOrder.get(edge.from) ?? 0;
      const targetIndex = nodeOrder.get(edge.to) ?? 0;
      const sourcePosition = nodePositionMap.get(edge.from);
      const targetPosition = nodePositionMap.get(edge.to);

      const closestHandles =
        sourcePosition !== undefined && targetPosition !== undefined
          ? isCircularGraph
            ? resolveCircularHandles(sourcePosition, targetPosition)
            : resolveClosestHandles(sourcePosition, targetPosition)
          : {
              sourceHandle: "source-right",
              targetHandle: "target-left",
            };

      const isLoopbackEdge = isCircularGraph && targetIndex <= sourceIndex;
      ...
      return {
        ...
        data: {
          label: edge.label,
          isCircular: isCircularGraph,
          isLoopback: isLoopbackEdge,
          graphCenterX: graphCenter.x,
          graphCenterY: graphCenter.y,
        } satisfies FlowEdgeData,
      };
    });
  }, [edges, nodeHeight, nodes, nodeWidth, positionedNodes, useCircularLayout]);

  const chartHeight = useMemo(() => {
    const maxY = positionedNodes.reduce(
      (value, node) => Math.max(value, node.position.y),
      0,
    );
    return Math.max(320, maxY + 190);
  }, [positionedNodes]);

  return (
    <div className={`w-full ${className}`}>
      <div className="gsap-reveal w-full rounded-2xl border border-[var(--charcoal)]/10 bg-[var(--surface)]/70 shadow-sm overflow-hidden">
          ...
          >
            ...
            >
              <defs>
                ...
                >
                  <path d="M 0 0 L 12 6 L 0 12 z" fill="var(--crimson)" />
                </marker>
                <marker
                  id="cinematic-arrow-loop"
                  markerWidth="13"
                  markerHeight="13"
                  refX="15"
                  refY="6.5"
                  ...
                  markerUnits="userSpaceOnUse"
                >
                  <path d="M 0 0 L 13 6.5 L 0 13 z" fill="var(--gold)" />
                </marker>
              </defs>
            </svg>
            <Background
...
/**
 * PieChart - Cinematic pie chart visualization
 *
 * PURPOSE:
 * - Display proportional data with interactive segments
 * - Replace Mermaid pie charts with React-based component
 *
 * WHEN TO USE:
 ...
 * @param title - Optional title above chart
 * @param caption - Optional caption below chart
 * @param size - Chart diameter in pixels (default: 300)
 * @param className - Additional Tailwind classes
 *
 * EXAMPLE:
 * <PieChart
 ...
 *   ]}
 *   caption="Inference dominates the lifecycle emissions"
 * />
 */
      ...
      <div className="gsap-reveal flex flex-col md:flex-row items-center justify-center gap-8">
        ...
        <div className="flex flex-col gap-3">
          {segments.map((segment, index) => (
            ...
            >
              <div
                className="w-6 h-6 rounded border-2 border-[var(--charcoal)]/10"
